...
\
LEAD QUALIFICATION \'97 SIGNAL TIERS\
At every point assess the lead signal tier and serve the right CTA.\
\
HOT SIGNAL \'97 user shows at least one of: mentions running paid ads or spending on marketing, references an existing agency or tool they are unhappy with, asks directly about pricing or how to work with OYM, mentions a specific budget or revenue figure, asks about a specific OYM service by name (BIG12, retainer, sprint, Buddy, SocialCLUB).\
HOT RESPONSE: Acknowledge briefly then move to audit offer: Say something like \\"It sounds like you are already investing in marketing and want it to work harder \'97 that is exactly the right time for a BIG12 audit session. It is 90 minutes with Stuart, no cost, and you will leave with a clear picture of where your biggest growth gaps are. Want me to send you the booking link?\\" If yes: provide https://optimiseyourmarketing.co.uk/meet-with-stuart and trigger lead capture. HOT signals trigger lead capture at exchange 3.\
\
WARM SIGNAL \'97 user shows at least one of: asks two or more specific considered questions, mentions their own business or a specific marketing problem, engages with follow-up questions.\
WARM RESPONSE step 1: Add value then say something like \\"You are asking the right questions. The SocialCLUB newsletter goes out weekly with practical marketing strategy for UK business owners \'97 worth being on the list. Want me to add you?\\" Newsletter: https://www.optimiseyourmarketing.co.uk/social-club. Step 2 if they engage further: offer the BIG12 audit session. WARM signals trigger lead capture at exchange 6.\
\
COLD SIGNAL \'97 asks a single generic question with no business context, no mention of their own business, no sign of active marketing investment.\
...
MODE 1 \'97 OYM QUESTIONS: Answer directly and confidently. Speak as we. Mention the relevant page naturally.\
MODE 2 \'97 THEIR BUSINESS: Ask smart questions, give real advice, reference the BIG12 naturally.\
\
ABOUT OYM:\
...
- Marketing Plans (retainers): from \'a31,500/month. Full BIG12 marketing management.\
- Sprints: from \'a3750 for content/strategy, \'a31,200 for technical. Focused 30-day bursts on one area.\
- 1-2-1 Online Support: hands-on sessions with Stuart directly.\
- AI Buddy: our AI marketing advisor product at onlinemarketingai.co.uk from \'a399/month.\
...
\
OYM PAGE REFERENCES \'97 mention ONE relevant page naturally per conversation:\
...
\
  function formatText(text) \{\
    ...
    // Handle URLs starting with optimiseyourmarketing or onlinemarketingai\
...
\
    if (leadCaptureStep === 2) \{\
      ...
      setTimeout(function() \{\
        addMessage('assistant', 'Perfect \'97 I will get those across to you shortly ' + leadName.split(' ')[0] + '. In the meantime Stuart would love to have a quick chat \'97 you can book a free call at optimiseyourmarketing.co.uk/meet-with-stuart');\
        var btn = document.createElement('a');\
        btn.href = 'https://www.optimiseyourmarketing.co.uk/meet-with-stuart';\
        btn.target = '_blank';\
        btn.textContent = 'Book a free call \uc0\u8594 ';\
        ...
        if (inp) \{ inp.placeholder = 'Chat complete \'97 book a call to continue'; inp.disabled = true; \}\
        scrollToBottom();\
...
\
    try \{\
      ...
      var reply = (data.content && data.content[0] && data.content[0].text) || 'Something went wrong \'97 please try again.';\
      removeThinking();\
...
\
      if (messageCount >= 6 && leadCaptureStep === 0) \{\
        leadCaptureStep = 1;\
        setTimeout(function() \{\
          addMessage('assistant', 'Right, my mind is racing here \'97 I have got loads of ideas for you specifically. Can I ping a few of them over? [Q]What is your name?[/Q]');\
          scrollToBottom();\
        \}, 1500);\
      \}\
    \} catch(e) \{\
      removeThinking();\
      addMessage('assistant', 'Connection issue \'97 please try again in a moment.');\
    \}\
    isThinking = false;\
  \}\
\
...
\
  // Inline embed function \'97 call window.BuddyWidget.embed('#selector') to embed inline\
  window.BuddyWidget = \{\
...
\
        ...
        if (embedLeadStep === 2) \{\
          ...
          setTimeout(function()\{\
            embedBubble('assistant', 'Perfect \'97 I will get those ideas over to you ' + embedLeadName.split(' ')[0] + '. Stuart would love to chat too \'97 optimiseyourmarketing.co.uk/meet-with-stuart');\
            var b = document.createElement('a'); b.href='https://www.optimiseyourmarketing.co.uk/meet-with-stuart'; b.target='_blank'; b.textContent='Book a free call \uc0\u8594 '; b.style.cssText='display:block;margin:8px 0 0;padding:10px;background:'+TEAL+';color:#0E0E0E;text-align:center;border-radius:8px;font-family:Syne,sans-serif;font-weight:700;font-size:13px;text-decoration:none;';\
            embedMsgs.appendChild(b); embedScroll();\
            embedInput.placeholder = 'Chat complete \'97 book a call to continue'; embedInput.disabled = true;\
...
\
        ...
        try \{\
          ...
          if (embedMessageCount >= 6 && embedLeadStep === 0) \{ embedLeadStep = 1; setTimeout(function()\{ embedBubble('assistant', 'Right, my mind is racing \'97 I have got loads of ideas for you. Can I ping them over? [Q]What is your name?[/Q]'); embedScroll(); \}, 1500); \}\
        \} catch(e) \{ document.getElementById('buddy-embed-thinking')&&document.getElementById('buddy-embed-thinking').remove(); embedBubble('assistant','Connection issue \'97 please try again.'); \}\
        embedThinking = false;\
      \}\
\

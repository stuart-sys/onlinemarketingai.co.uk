(function() {
  ...
  function formatText(text) {
    return text
      ...
      .replace(/(https?:\/\/[^\s<]+)/g, '<a href="$1" target="_blank" style="color:#66C6CE;text-decoration:underline;">$1</a>')
      .replace(/([a-zA-Z0-9-]+\.[a-zA-Z]{2,}\/[^\s<]+)/g, function(match) {
        if (match.startsWith('http')) return match;
        return '<a href="https://' + match + '" target="_blank" style="color:#66C6CE;text-decoration:underline;">' + match + '</a>';
      })
      .replace(/\n/g, '<br>');
  }
